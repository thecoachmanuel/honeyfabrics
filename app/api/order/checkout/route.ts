...
export async function POST(req: NextRequest) {
  try {
    ...
    let { items, customerName, name, email, phone, address, note, deliveryMethod, deliveryFee, createAccount, password } = body

    customerName = customerName || name

    if (!items || items.length === 0) return NextResponse.json({ error: 'Cart is empty' }, { status: 400 })
    if (!customerName || !email || !phone) return NextResponse.json({ error: 'Missing contact info' }, { status: 400 })

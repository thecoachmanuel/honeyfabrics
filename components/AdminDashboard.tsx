...
export default function AdminDashboard({ settings, categories, products, slides, orders, messages = [], notifications = [], reviews = [], deliverySettings, initialTab }: AdminDashboardProps) {
  ...
  const exportOrdersCSV = () => {
    const lines = [
      `Site,${settings?.businessName || 'Cakes by Deejah'}`,
      `Report,Order Report`,
  ...
  const exportOrdersPDF = () => {
    ...
    doc.setFontSize(22)
    doc.setTextColor(40)
    doc.text(settings?.businessName || 'Cakes by Deejah', 14, 20)
  ...
  const deleteReview = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) setLocalReviews(prev => prev.filter(r => r.id !== id))
  ...
  const saveReview = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('/api/admin/reviews', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editingReview.id, comment })
      })
      if (res.ok) {
        setLocalReviews(prev => prev.map(r => r.id === editingReview.id ? { ...r, comment } : r))
  ...
  const exportProductsCSV = () => {
    const lines = [
      `Site,${settings?.businessName || 'Honey Fabrics'}`,
  ...
  return (
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <main className="flex-1 min-w-0">
          ...
          <div className="space-y-6">
            ...
            <form key={editingProduct?.id || 'new-product'} className="card p-6" onSubmit={saveProduct}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-cocoa">Stock Count</label>
                  <input name="stock" type="number" defaultValue={editingProduct?.stock || 0} required className="input w-full border rounded p-2" />
                </div>
                <div className="space-y-2">
                ...
                {products.map(p => (
                  <div key={p.id} className="border rounded p-3 flex gap-3 items-center group relative">
                    ...
                    <div className="overflow-hidden flex-1">
                      ...
                      <p className="text-sm text-cocoa/70">{formatNgn(p.priceNgn)} • Stock: {p.stock || 0}</p>
                    </div>
        ...
        {activeTab === 'settings' && (
          <form onSubmit={saveSettings} className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-cocoa">Twitter Handle</label>
                <input name="twitter" defaultValue={settings?.twitter ?? ''} placeholder="@handle" className="input w-full border rounded p-2" />
              </div>
              <div className="space-y-2">
        ...
        {activeTab === 'reviews' && (
          <div className="card p-6">
            <h2 className="text-xl font-bold text-cocoa mb-4">Customer Reviews ({localReviews.length})</h2>
            <div className="space-y-4">
              {localReviews.map((r: any) => (
                <div key={r.id} className="border rounded p-4 relative group">
                  <div className="flex justify-between items-start mb-2">
                     <div>
                       <p className="font-bold text-cocoa">{r.user?.name || 'Anonymous'}</p>
                       <p className="text-xs text-cocoa/60">on {r.product?.name || 'Unknown Product'}</p>
                     </div>
                  ...
                  {editingReview?.id === r.id ? (
                    <form onSubmit={saveReview} className="mt-2">
                      <textarea name="comment" defaultValue={r.comment} className="w-full border rounded p-2 text-sm" rows={3} />
                      <div className="flex gap-2 mt-2">
                        <button className="btn btn-primary text-xs py-1 px-3 h-auto min-h-0">Save</button>
                        <button type="button" onClick={() => setEditingReview(null)} className="btn btn-outline text-xs py-1 px-3 h-auto min-h-0">Cancel</button>
                      </div>
                    </form>
                  ) : (
                  ...
                  <div className="mt-2 flex justify-between items-center">
                    <span className="text-[10px] text-cocoa/40">{new Date(r.createdAt).toLocaleString()}</span>
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => setEditingReview(r)} className="text-xs text-caramel hover:underline">Edit</button>
                      <button onClick={() => deleteReview(r.id)} className="text-xs text-red-500 hover:underline">Delete</button>
                    </div>
                  </div>
                </div>
              ))}
              {localReviews.length === 0 && <p className="text-center text-cocoa/50 py-8">No reviews yet.</p>}
            </div>
          </div>
        )}
